...
class CalculAvancementService {
  ...
  static const Map<String, double> _baremeSemiEnterre = {
    ...
    'sec11': 4.0, // DLM
    'sec12': 0.0, // Garde-fous (non applicable pour semi-enterré ou = 0 selon tableau)
  };
  ...
  static double calculerAvancement(String jsonControleTravaux, String typeLatrine) {
    if (jsonControleTravaux.isEmpty) return 0.0;

    final bareme = (typeLatrine == 'Semi-enterrée') ? _baremeSemiEnterre : _baremeHorsSol;
    ...
    try {
      ...
      if (donnees['section1'] != null) {
        if (donnees['section1']['acheve'] == true) {
          ...
          avancementTotal += bareme['sec1']! / 2; // Arbitrairement la moitié si en cours
        }
      }
      ...
      avancementTotal += _verifierListeSimple(donnees['section6'], bareme['sec6']!);
      ...
      avancementTotal += _verifierListeSimple(donnees['section9'], bareme['sec9']!);

      // Section 10 : Revêtement (simple)
      avancementTotal += _verifierListeSimple(donnees['section10'], bareme['sec10']!);

      // Section 11 : DLM
      avancementTotal += _verifierSectionListe(donnees['section11'], bareme['sec11']!);

      // Section 12 : Garde-fous (simple)
      avancementTotal += _verifierListeSimple(donnees['section12'], bareme['sec12']!);
  ...
  static double _verifierSectionListe(Map<String, dynamic>? sectionData, double pointsMaximum) {
    if (sectionData == null || sectionData['aposteriori'] == null) return 0.0;

    List<dynamic> aposteriori = sectionData['aposteriori'];
    if (aposteriori.isEmpty) return 0.0;
  ...
  static double _verifierListeSimple(List<dynamic>? sectionList, double pointsMaximum) {
    if (sectionList == null || sectionList.isEmpty) return 0.0;
